 ...
 */
public class InstGhost extends ParallacticAngleSupportInst
                       implements IssPortProvider, PosAngleConstraintAware,
                                  StepCalculator, VignettableScienceAreaInstrument,
                                  PropertyProvider, CalibrationKeyProvider, ItcOverheadProvider {
    ...
    public static final String AMP_COUNT_PROP_NAME = GhostType.AmpCount.KEY.getName();
    public static final String DETECTOR_MANUFACTURER_PROP_NAME = GhostType.DetectorManufacturer.KEY.getName();
    ...
    public static final ISPNodeInitializer<ISPObsComponent, InstGmosNorth> NI =
            new ComponentNodeInitializer<>(SP_TYPE, () -> new InstGmosNorth(), c -> new InstGMOSCB(c));
    ...
    static {
        ...
        try {
            AMP_GAIN_SETTING_PROP = new PropertyDescriptor("gainSetting", InstGhost.class, "getActualGain", null);
        ...
        AMP_READ_MODE_PROP = initProp(GhostType.AmpReadMode.KEY.getName(), false, true);
        AMP_READ_MODE_PROP.setDisplayName("Amp Read Mode");

        BUILTIN_ROI_PROP = initProp(GhostType.BuiltinROI.KEY.getName(), query_yes, iter_yes);
        BUILTIN_ROI_PROP.setDisplayName("Builtin ROI");
        try {
            CUSTOM_ROI_PROP = new PropertyDescriptor("customROIs", InstGhost.class, "getCustomROIs", "setCustomROIs");
        ...
        DETECTOR_MANUFACTURER_PROP = initProp(InstGhost.DETECTOR_MANUFACTURER_PROP_NAME, false, false);
        ...
        try {
            FPU_MODE_PROP = new PropertyDescriptor("fpuMode", InstGhost.class, "getFPUnitMode", "setFPUnitMode");
        ...
        try {
            FPU_MASK_PROP = new PropertyDescriptor("fpuCustomMask", InstGhost.class, "getFPUnitCustomMask", "setFPUnitCustomMask");
        ...
        try {
            SHUFFLE_OFFSET_PROP = new PropertyDescriptor("nsShuffleOffset", InstGhost.class, "getShuffleOffset", "setShuffleOffset");
        ...
        try {
            DETECTOR_ROWS_PROP = new PropertyDescriptor("nsDetectorRows", InstGhost.class);
    ...
    private GhostType.ADC _adc = GhostType.ADC.DEFAULT;
    protected GhostType.AmpReadMode _ampReadMode = GhostType.AmpReadMode.DEFAULT;
    private GhostType.AmpCount _ampCount;
    protected GhostType.AmpGain _gainChoice = GhostType.AmpGain.DEFAULT;
    //    private GhostType.AmpGainReadCombo _gainReadCombo = GhostType.AmpGainReadCombo.SLOW_LOW;
    private GhostType.DetectorManufacturer _detectorManufacturer = GhostType.DetectorManufacturer.DEFAULT;
    //    private DET _detectorManufacturer;
    //    private GMOSParams.DisperserData _disperser = new GMOSParams.DisperserData();
    private GhostType.Disperser _disperser;
    private GhostType.Order _disperserOrder = GhostType.Order.DEFAULT;
    private double _disperserLambda = DEFAULT_DISPERSER_LAMBDA;

    private GhostType.FPUnitMode _fpuMode = GhostType.FPUnitMode.DEFAULT;
    private GhostType.CustomSlitWidth _customSlitWidth = GhostType.CustomSlitWidth.OTHER;
    //    private GMOSParams.FPUnit _fpu = GMOSParams.FPUnit.DEFAULT;
    private  GhostType.FPUnit _fpu;
    private String _fpuMaskLabel = EMPTY_STRING;
    //    private GMOSParams.Filter _filter = GMOSParams.Filter.DEFAULT;
    private GhostType.Filter _filter;
    ...
    //    private GhostType.StageMode _stageMode = GhostType.StageMode.DEFAULT;
    private GhostType.StageMode _stageMode;
    private GhostType.BuiltinROI _builtinROI = GhostType.BuiltinROI.DEFAULT;
    private GhostType.CustomROIList _customROIs = GhostType.CustomROIList.create();

    private IssPort _port = IssPort.SIDE_LOOKING;
    private GhostType.DTAX _dtaOffset = GhostType.DTAX.DEFAULT; // Detector translation assembly (DTA-X) offset
...
//    private double _pixelSize = GMOS_E2V_PIXEL_SIZE;
    ...
    static {
        InstGhost.nf.setMaximumFractionDigits(2);
    }

    // Gain setting.
    public static final ConfigInjector<String> GAIN_SETTING_INJECTOR = ConfigInjector.create(
            new ConfigInjectorCalc3<GhostType.AmpGain, GhostType.AmpReadMode, GhostType.DetectorManufacturer, String>() {
                ...
                @Override
                public String apply(GhostType.AmpGain gain, GhostType.AmpReadMode mode, GhostType.DetectorManufacturer man) {
    ...
    public InstGhost(SPComponentType type) {
        ...
        //  _updateDetectorRows();
        _detectorRows =  _validateDetectorRows(GhostType.DetectorManufacturer.DEFAULT.shuffleOffsetPixels());

        setDetectorManufacturer(GhostType.DetectorManufacturer.DEFAULT);

        setDisperser(GhostType.Disperser.DEFAULT);
        setFilter(GhostType.Filter.DEFAULT);
        setFPUnit(GhostType.FPUnit.DEFAULT);
        setStageMode(GhostType.StageMode.DEFAULT);
    }
    ...
    private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        if (_fpuMode == GhostType.FPUnitMode.CUSTOM_MASK) {

            _fpu =  GhostType.FPUnit.CUSTOM_MASK;
        }

        if (_posList != null) {
            _posList.addWatcher(new OffsetPosListChangePropagator<>(new InstGhost.PceNotifier(), _posList));
        }
    }
     ...
     * Set the ADC.
     */
    public void setAdc(GhostType.ADC newValue) {
        GhostType.ADC oldValue = getAdc();
    ...
    private void _setADC(String name) {
        GhostType.ADC oldValue = getAdc();
        setAdc(GhostType.ADC.getADC(name, oldValue));
    }

    /**
     * Return the current ccd amp readout speed.
     */
    public GhostType.AmpReadMode getAmpReadMode() {
        return _ampReadMode;
    }
    ...
    public double getActualGain() {
        final GhostType.AmpGain gain = getGainChoice();
        final GhostType.AmpReadMode readMode = getAmpReadMode();
        final GhostType.DetectorManufacturer detectorManufacturer = getDetectorManufacturer();
    ...
    public static int getActualGain(final GhostType.AmpGain gain,
                                    final GhostType.AmpReadMode readMode,
                                    final GhostType.DetectorManufacturer detectorManufacturer) {
        ...
        if (detectorManufacturer == GhostType.DetectorManufacturer.E2V) {
            if (readMode == GhostType.AmpReadMode.FAST) {
                if (gain == GhostType.AmpGain.HIGH) {
                    return 5;
                } else if (gain == GhostType.AmpGain.LOW) {
                    return 6;
                }
            } else if (readMode == GhostType.AmpReadMode.SLOW) {
                if (gain == GhostType.AmpGain.HIGH) {
                    return 1;
                } else if (gain == GhostType.AmpGain.LOW) {
                    return 2;
        ...
        } else if (detectorManufacturer == GhostType.DetectorManufacturer.HAMAMATSU) {
            if (readMode == GhostType.AmpReadMode.FAST) {
                if (gain == GhostType.AmpGain.HIGH) {
                    return 5;
                } else if (gain == GhostType.AmpGain.LOW) {
     ...
     */
    public double getMeanGain() {return getMeanGain(getGainChoice(), getAmpReadMode(), getDetectorManufacturer());}


    public static double getMeanGain(final GhostType.AmpGain gain,
                                     final GhostType.AmpReadMode readMode,
                                     final GhostType.DetectorManufacturer detectorManufacturer) {
     ...
     */
    public static double getMeanReadNoise(final GhostType.AmpGain gain,
                                          final GhostType.AmpReadMode readMode,
                                          final GhostType.DetectorManufacturer detectorManufacturer) {
     ...
     */
    public void setAmpReadMode(GhostType.AmpReadMode newValue) {
        GhostType.AmpReadMode oldValue = getAmpReadMode();
    ...
    protected void _setAmpReadMode(String name) {
        GhostType.AmpReadMode oldValue = getAmpReadMode();
        setAmpReadMode(GhostType.AmpReadMode.getAmpReadMode(name, oldValue));
    }

    /**
     * Return the current CCD amp count choice.
     */
    public GhostType.AmpCount getAmpCount() {
        return _ampCount;
    }
     ...
     */
    public void setAmpCount(GhostType.AmpCount newValue) {
        GhostType.AmpCount oldValue = getAmpCount();
        if (newValue != oldValue) {
            _ampCount = newValue;
    ...
    public void setAmpCount(String name) {
        GhostType.AmpCount oldValue = getAmpCount();
        GhostType.AmpCount newValue = GhostType.AmpCount.getAmpCount(name, oldValue);
     ...
     * Get the disperser.
     */
    public GhostType.Disperser getDisperser() {
        return _disperser;
    }
     ...
     */
    public void setDisperser(GhostType.Disperser newDisperser) {
        GhostType.Disperser oldDisperser = getDisperser();
    ...
    private void _setDisperser(String name) {
        GhostType.Disperser oldValue = getDisperser();
        GhostType.Disperser newValue = GhostType.Disperser.getDisperser(name, oldValue);
        setDisperser(newValue);
    }

    public GhostType.Order getDisperserOrder() {
        return _disperserOrder;
    }

    public void setDisperserOrder(GhostType.Order newValue) {
        GhostType.Order oldValue = _disperserOrder;
        if (oldValue != newValue) {
    ...
    private void _setDisperserOrder(String order) {
        setDisperserOrder(GhostType.Order.getOrder(order));
    }
     ...
     * Get the FPUnit.
     */
    public GhostType.FPUnit getFPUnit() {
        if (_fpu == null) {
            //noinspection unchecked
            _fpu = GhostType.FPUnit.DEFAULT;
        }
        return _fpu;
    }
     ...
     * Set the FPUnit.
     */
    public void setFPUnit(GhostType.FPUnit  newValue) {
        GhostType.FPUnit oldValue = getFPUnit();
        if (oldValue != newValue) {
            _fpu = newValue;
        ...
        GhostType.FPUnit customMaskFPU = GhostType.FPUnit.CUSTOM_MASK;
        GhostType.FPUnitMode customMaskMode = GhostType.FPUnitMode.CUSTOM_MASK;
        if (_fpu == customMaskFPU && _fpuMode != customMaskMode) {
            ...
            setFPUnitMode(GhostType.FPUnitMode.BUILTIN);
        }
    }
    ...
    protected void _setFPUnit(String name) {
        GhostType.FPUnit oldValue = getFPUnit();
        //noinspection unchecked
        GhostType.FPUnit newValue = GhostType.FPUnit.getFPUnit(name, oldValue);
     ...
     */
    public GhostType.FPUnitMode getFPUnitMode() {
        return _fpuMode;
    }
     ...
     */
    public void setFPUnitMode(GhostType.FPUnitMode newValue) {
        GhostType.FPUnitMode oldValue = _fpuMode;
        if (oldValue != newValue) {
            _fpuMode = newValue;
        ...
        //noinspection unchecked
        GhostType.FPUnit customMaskFPU = GhostType.FPUnit.DEFAULT;
        GhostType.FPUnitMode customMaskMode = GhostType.FPUnitMode.CUSTOM_MASK;
        if (_fpuMode == customMaskMode && _fpu != customMaskFPU) {
            setFPUnit(customMaskFPU);
        } else if (_fpuMode == GhostType.FPUnitMode.BUILTIN && _fpu == customMaskFPU) {
            //noinspection unchecked
            setFPUnit(GhostType.FPUnit.FPU_NONE);
        }
    }

    private void _setFPUnitMode(String name) {
        GhostType.FPUnitMode oldValue = getFPUnitMode();
        setFPUnitMode(GhostType.FPUnitMode.getFPUnitMode(name, oldValue));
    }
    ...
    public boolean isCustomMaskInUse() {
        return getFPUnitMode() == GhostType.FPUnitMode.CUSTOM_MASK;
    }

    /**
     * Get the filter.
     */
    public GhostType.Filter getFilter() {
        return _filter;
    }
     ...
     * Set the filter.
     */
    public void setFilter(GhostType.Filter newValue) {
        GhostType.Filter oldValue = getFilter();
        if (oldValue != newValue) {
            _filter = newValue;
    ...
    protected void _setFilter(String name) {
        GhostType.Filter oldValue = getFilter();
        //noinspection unchecked
        GhostType.Filter newValue = GhostType.Filter.getFilter(name, oldValue);
        setFilter(newValue);
    }
     ...
     */
    public void setStageMode(GhostType.StageMode newValue) {
        GhostType.StageMode oldValue = getStageMode();
        if (newValue != oldValue) {
            _stageMode = newValue;
     ...
     */
    protected void _setStageMode(GhostType.StageMode newValue) {
        _stageMode = newValue;
    }
    ...
    private void _setStageMode(String name) {
        ...
        //noinspection unchecked
        GhostType.StageMode newValue = GhostType.StageMode.getStageMode(name, oldValue);
     ...
     */
    public GhostType.StageMode getStageMode() {
        return _stageMode;
    }
     ...
     * detector.
     */
    public void setBuiltinROI(GhostType.BuiltinROI newValue) {
        GhostType.BuiltinROI oldValue = getBuiltinROI();
        if (newValue != oldValue) {
    ...
    private void _setBuiltinROI(String name) {
        GhostType.BuiltinROI oldValue = getBuiltinROI();
        setBuiltinROI(GhostType.BuiltinROI.getBuiltinROI(name, oldValue));
    }

    /**
     * Return the current value for the stage mode.
     */
    public GhostType.BuiltinROI getBuiltinROI() {
        return _builtinROI;
    }

    public GhostType.CustomROIList getCustomROIs() {
        return _customROIs;
    }

    public boolean validateCustomROIs() {
        for (GhostType.ROIDescription roi : getCustomROIs().get()) {
    ...
    public void setCustomROIs(final GhostType.CustomROIList customROIs) {
        final GhostType.CustomROIList oldValue = _customROIs;
    ...
    public GhostType.UseNS getUseNS() {
        return useNS() ? GhostType.UseNS.TRUE : GhostType.UseNS.FALSE;
    }
    ...
    public void setUseNS(GhostType.UseNS useNS) {
    ...
    private void _setUseNS(String name) {
        GhostType.UseNS oldValue = getUseNS();
        setUseNS(GhostType.UseNS.getUseNS(name, oldValue));
    }
    ...
    public OffsetPosList<OffsetPos> getPosList() {
        if (_posList == null) {
            ...
            _posList.addWatcher(new OffsetPosListChangePropagator<>(new InstGhost.PceNotifier()));
        }
        return _posList;
    }
...
//            _updateDetectorRows();
    ...
    public CategorizedTimeGroup calc(Config cur, Option<Config> prev) {
        ...
        if (PlannedTime.isUpdated(cur, prev, GhostType.FPUnit.KEY)) {
            times.add(CategorizedTime.fromSeconds(
                    Category.CONFIG_CHANGE, GhostType.FPUnit.CHANGE_OVERHEAD, "FPU")
            );
        }
        if (PlannedTime.isUpdated(cur, prev, GhostType.Filter.KEY)) {
            times.add(CategorizedTime.fromSeconds(
                    Category.CONFIG_CHANGE, GhostType.Filter.CHANGE_OVERHEAD, "Filter")
            );
        }
        if (PlannedTime.isUpdated(cur, prev, GhostType.Disperser.KEY)) {
            times.add(CategorizedTime.fromSeconds(
                    Category.CONFIG_CHANGE, GhostType.Disperser.CHANGE_OVERHEAD, "Disperser")
            );
        }

        double readoutTime = GhostReadoutTime.getReadoutOverhead(cur, getCustomROIs());
    ...
    public void updateConfig(IConfig config) {
        ...
        config.getParameterValue(s, InstGhost.AMP_COUNT_PROP_NAME);
        config.getParameterValue(s, InstGhost.BUILTIN_ROI_PROP.getName());
        config.getParameterValue(s, InstGhost.CCD_X_BIN_PROP.getName());
        config.getParameterValue(s, InstGhost.CCD_Y_BIN_PROP.getName());
        config.getParameterValue(s, InstGhost.AMP_READ_MODE_PROP.getName());
        config.getParameterValue(s, InstGhost.AMP_GAIN_CHOICE_PROP.getName());
    }

    /**
     * Return the Detector translation assembly (DTA-X) offset.
     */
    public GhostType.DTAX getDtaXOffset() {
        return _dtaOffset;
    }
     ...
     */
    public void setDtaXOffset(GhostType.DTAX newValue) {
        GhostType.DTAX oldValue = _dtaOffset;
        if (newValue != oldValue) {
            _dtaOffset = newValue;
     ...
     * Set the Detector translation assembly (DTA-X) offset and fire an event.
     */
    protected void _setDtaXOffset(GhostType.DTAX newValue) {
        _dtaOffset = newValue;
    }
    ...
    private void _setDtaXOffset(String name) {
        GhostType.DTAX oldValue = getDtaXOffset();
        setDtaXOffset(GhostType.DTAX.getDTAX(name, oldValue));
    }

    /**
     * Gets the custom slit width
     */
    public GhostType.CustomSlitWidth getCustomSlitWidth() {
     ...
     */
    public void setCustomSlitWidth(GhostType.CustomSlitWidth newValue) {
        GhostType.CustomSlitWidth oldValue = getCustomSlitWidth();
        if (oldValue != newValue) {
...
//        Pio.addParam(factory, paramSet, ENGINEERING_AMP_GAIN_READ_COMBO_PROP.getName(), getEngineeringGainReadCombo().name());
        ...
        GhostType.FPUnitMode fpuMode = getFPUnitMode();
        ...
        if (fpuMode == GhostType.FPUnitMode.CUSTOM_MASK) {
            // It's a custom mask
    ...
    public void setParamSet(ParamSet paramSet) {
        ...
        if (getDisperser().isMirror()) {
            setDisperserOrder(GhostType.Order.DEFAULT);
        ...
        if (getFPUnitMode() == GhostType.FPUnitMode.BUILTIN) {
            v = Pio.getValue(paramSet, FPU_PROP_NAME);
            if (v != null) _setFPUnit(v);
        } else if (getFPUnitMode() == GhostType.FPUnitMode.CUSTOM_MASK) {
            ...
            if (v != null) {
                setCustomSlitWidth(GhostType.CustomSlitWidth.valueOf(v));
            }
        }
        ...
        if (p != null) {
            try {
                setCustomROIs(GhostType.CustomROIList.create(p));
    ...
    public ISysConfig getSysConfig() {
        ...
        if (fpuMode == GhostType.FPUnitMode.BUILTIN) {
    ...
    public void setDetectorManufacturer(final String newValue) {
        setDetectorManufacturer(GhostType.DetectorManufacturer.valueOf(newValue));
    }

    /**
     * Return the current CCD manufacturer.
     */
    public GhostType.DetectorManufacturer getDetectorManufacturer() {
     ...
     */
    public void setDetectorManufacturer(GhostType.DetectorManufacturer newValue) {
        final GhostType.DetectorManufacturer oldValue = getDetectorManufacturer();
        if (newValue != oldValue) {
    ...
    public void initializeDetectorManufacturerValues() {
        final GhostType.DetectorManufacturer dm = getDetectorManufacturer();

        if (dm == GhostType.DetectorManufacturer.E2V) {
            setAmpCount(defaultAmpCountForE2V());
        } else if (dm == GhostType.DetectorManufacturer.HAMAMATSU) {
            setAmpCount(GhostType.AmpCount.TWELVE);
        } else {
            throw new IllegalArgumentException("Detector " + dm.name() + " not valid for GMOS");
        }
    }

    protected GhostType.AmpCount defaultAmpCountForE2V() {
        return GhostType.AmpCount.SIX;
    }
...
//    public GhostType.AmpGainReadCombo getGainReadCombo() {
//        return _gainReadCombo;
//    }
//
//    private void _setGainReadCombo(String name) {
//        final GhostType.AmpGainReadCombo readCombo = GhostType.AmpGainReadCombo.getAmpGainReadCombo(name);
//        setGainReadCombo(readCombo);
//    }
//
//    public void setGainReadCombo(GhostType.AmpGainReadCombo newValue) {
//        GhostType.AmpGainReadCombo oldValue = getGainReadCombo();
//        if (newValue != oldValue) {
...
//    }
    ...
    public static String calcWavelength(GhostType.Disperser d, GhostType.Filter f, Double centralWavelength) {
    ...
    public static final class UseElectronicOffsettingRuling {
        ...
        public static InstGhost.UseElectronicOffsettingRuling allow() {
            return new InstGhost.UseElectronicOffsettingRuling(true, "");
        }

        public static InstGhost.UseElectronicOffsettingRuling deny(String message) {
            return new InstGhost.UseElectronicOffsettingRuling(false, message);
        }
    }
     ...
     */
    public static InstGhost.UseElectronicOffsettingRuling checkUseElectronicOffsetting(InstGhost inst, OffsetPosList<OffsetPos> p) {
        int size = p.size();
        if (size == 0) {
            return InstGhost.UseElectronicOffsettingRuling.deny("There are no offset positions defined.");
        }
        ...
        if (dist > 2) {
            return InstGhost.UseElectronicOffsettingRuling.deny("Max electronic offset distance of 2 arcsec exceeded");
        }

        // must be okay then
        return InstGhost.UseElectronicOffsettingRuling.allow();
    }
    ...
    public boolean isCompatibleWithMeanParallacticAngleMode() {
        return !(_fpu.isImaging() || _fpuMode == GhostType.FPUnitMode.CUSTOM_MASK);
    }
    ...
    public boolean allowUnboundedPositionAngle() {
        ...
        boolean isCustom = getFPUnitMode() == GhostType.FPUnitMode.CUSTOM_MASK;
        boolean isNone   = getFPUnit() == GhostType.FPUnit.FPU_NONE;
    ...
    public void restoreScienceDetails(final SPInstObsComp oldData) {
        ...
        if (oldData instanceof InstGhost) {
            final InstGhost oldGmos = (InstGhost) oldData;
    ...
    public double[] getScienceArea() {
        ...
        //return GhostScienceAreaGeometry.javaScienceAreaDimensions(1.0);  //It is necessary to fill the GhostScienceAreaGeometry and removes the belove code
        double[] result = {330.34, 330.34};
        if (_fpu.getWidth() != -1) {
            result[0] = _fpu.getWidth();
            result[1] = 330.34;
        }
